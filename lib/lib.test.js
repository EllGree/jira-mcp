...
test("a wiki-style mention becomes a mention node, not literal text", () => {
  const doc = textToAdf("[~accountid:712020:abc-1] please check", { "712020:abc-1": "Jane" });
  const [p] = doc.content;
  assert.deepEqual(p.content[0], { type: "mention", attrs: { id: "712020:abc-1", text: "@Jane" } });
...
test("@[id|Name] carries its own name and needs no lookup", () => {
  assert.deepEqual(mentionIds("@[a1|Jane Doe] and @[b2]"), ["b2"]);
  const [p] = textToAdf("@[a1|Jane Doe] ping", {}).content;
  assert.deepEqual(p.content[0], { type: "mention", attrs: { id: "a1", text: "@Jane Doe" } });
});
