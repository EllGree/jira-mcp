...
//   @[ID]  @[ID|Display Name]  [~accountid:ID]   a mention; without a name, the caller looks it up
//   [label](https://...)         a link
//   https://...                  a bare link
//   `code`                       inline code
//   **bold**                     strong
...
/** The account ids mentioned in `text` without a display name, so the caller can look those up first. */
export function mentionIds(text) {
  const ids = new Set();
  for (const m of String(text).matchAll(INLINE)) {
    if ((m[1] || m[2]) && !m[3]) ids.add(m[1] || m[2]);
  }
  return [...ids];
}
...
export function inline(line, names = {}) {
  ...
  for (const m of line.matchAll(INLINE)) {
    ...
    const [, accA, accB, given, label, href, bare, code, bold] = m;
    const id = accA || accB;
    if (id) {
      const name = given?.trim() || names[id];
